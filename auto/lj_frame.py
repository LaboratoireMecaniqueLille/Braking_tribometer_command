...
class LJFrame(tk.Frame):
  def __init__(self,root):
    ...
    self.listbox.grid(row=0,column=3,rowspan=4,columnspan=2)
    ...
    self.del_button.grid(row=5,column=4)
    self.chan_list = []
    ...
    tk.Label(self,text="Offset").grid(row=4,column=0)
    self.t_offset = tk.Entry(self)
    self.t_offset.grid(row=4,column=1)

    tk.Label(self,text="Make zero?").grid(row=5,column=0)
    self.c_zero_var = tk.IntVar()
    self.c_zero = tk.Checkbutton(self,variable=self.c_zero_var)
    self.c_zero.grid(row=5,column=1)
